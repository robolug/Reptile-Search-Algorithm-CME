...
%         thi = robots(j).Itertn(i).Pose(1,3);        
        ...
        if j==1
            ...
            hold on
        elseif j==2
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(2),'marker','p');
            hold on
        elseif j==3
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(3),'marker','*');
            hold on
        end
        show(map)
        grid on
...
try 
    ...
    %         rbtbest_vl.value = [];
            ...
            for j=1:nRbt
                ...
                Dist(1,:) = abs(C1.*getOccupancy(map,wolf(j).Itertn(i).alpha.Position) - getOccupancy(map,robots(j).Itertn(i).Pose(1,:)));
                Dist(2,:) = abs(C2.*getOccupancy(map,wolf(j).Itertn(i).beta.Position) - getOccupancy(map,robots(j).Itertn(i).Pose(1,:)));
