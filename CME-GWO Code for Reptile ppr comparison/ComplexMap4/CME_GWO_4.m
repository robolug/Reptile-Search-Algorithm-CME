...
% 
%%
block_height = 3.0; % Increase the height
vertices = [
    0, 0, 0;            % Vertex 1
    1.75, 0, 0;          % Vertex 2
    1.75, 1.75, 0;        % Vertex 3
    0, 1.75, 0;          % Vertex 4
    0, 0, block_height; % Vertex 5 (Increased height)
    1.75, 0, block_height;% Vertex 6 (Increased height)
    1.75, 1.75, block_height;% Vertex 7 (Increased height)
    0, 1.75, block_height;% Vertex 8 (Increased height)
];
...
p1 =  [2,7]; p2 = [2,8]; p3 = [2,9]; p4 = [2,10]; p5 = [2,11]; p6 = [2,12]; 
p7=[2,13]; p8 =[2,14]; p9=[2,15]; p10=[2,16]; p11=[2,17];   

p13 = [18,7]; p14 = [18,8]; p15 = [18,9]; p16 = [18,10]; p17 = [18,11]; p18 = [18,12]; 
p19_=[18,13]; p19 =[18,14]; p20=[18,15]; p21=[18,15]; p22=[18,16];  p23=[18,17]; 


positions = [p1; p2; p3; p4;p6 ;p7;p8; p9;p10;p11; p13;p14;p15;p16;p17;p18;p19_;p19;p20; p21;p22;p23];
%positions_ = [11, 17, 0; 12, 16, 0; 13,17,0 ;14, 16,0; 15,11,0];  % Adjust positions as needed

positions_ = [2, 7, 0; 2,8,0; 2,9,0 ;2, 10,0; 2,11,0; 2,12,0 ; 2,13,0; 2,14,0; 2,15,0;...
   2,16,0; 2,17,0;....
   18,7,0; 18,8,0; 18,9,0; 18,10,0; 18,11,0; 18,12,0; 18,13,0; 18,14,0; 18,15,0; 18,16,0; 18,17,0 ];
...
%         thi = robots(j).Itertn(i).Pose(1,3);        
        ...
        if j==1
            ...
            hold on
        elseif j==2
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(2),'marker','+');
            hold on
        elseif j==3
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(3),'marker','d');
            hold on
        end
        show(map)
        grid on
...
try 
    ...
    %         rbtbest_vl.value = [];
            ...
            for j=1:nRbt
                ...
                Dist(1,:) = abs(C1.*getOccupancy(map,wolf(j).Itertn(i).alpha.Position) - ...
                    getOccupancy(map,robots(j).Itertn(i).Pose(1,:)));
                Dist(2,:) = abs(C2.*getOccupancy(map,wolf(j).Itertn(i).beta.Position) - ...
                    getOccupancy(map,robots(j).Itertn(i).Pose(1,:)));
                Dist(3,:) = abs(C3.*getOccupancy(map,wolf(j).Itertn(i).gamma.Position) - getOccupancy(map,robots(j).Itertn(i).Pose(1,:)));
                wolf(j).Itertn(i).alpha.X1 = getOccupancy(map,wolf(j).Itertn(i).alpha.Position) - A1.*Dist(1,:);
                wolf(j).Itertn(i).beta.X2 = getOccupancy(map,wolf(j).Itertn(i).beta.Position) - A2.*Dist(2,:);
                wolf(j).Itertn(i).gamma.X3 = getOccupancy(map,wolf(j).Itertn(i).gamma.Position) - A3.*Dist(3,:);

                temp_arr = [wolf(j).Itertn(i).alpha.X1, wolf(j).Itertn(i).beta.X2, wolf(j).Itertn(i).gamma.X3];
        ...
        disp("Parameters: " + "A1 = " + A1 + " A2 = " + A2 + " A3 = " + A3 + " C1 = " + C1 +...
            " C2 = " + C2 + " C3 = " + C3 + " a = " + a);
...
for row = 2:bound-2
    for column = 2:bound-2
        ...
        elseif row == 4 && (column == 4)
            Utility_covered;
        elseif row == 5 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 6 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 7 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 8 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 9 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 10 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 11 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 12 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 13 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 14 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 15 && (column == 8 || column == 9 || column == 10)
            Utility_covered;
        elseif row == 16 && column == 9
            Utility_covered;
        else
...
for j=1:nRbt
        for it_=1:i
            ...
            if j==1
                plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),'color',color(1),'marker','s');
                hold on

            elseif j==2
                plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),'color',color(2),'marker','+');
                hold on
...
%% MS Excel file generation
   
% a=0;
% for i=1:10
% time=a+2;
% ExploredAreaPercentag=a+4;
% p(a+1,:)=[time ExploredAreaPercentag];
% xlswrite('mir.xls',p)
% a=a+1;
% end
...
% Initialize arrays to store time and exploration rate data
timeData = zeros(1, 100);
explorationRateData = zeros(1, 100);

% Simulate your 100 iterations
for iteration = 1:100
  
    simulatedTime = toc; % Replace with actual time calculation
    explorationRate = ExploredAreaPercentage; % Replace with actual exploration rate calculation
end    
    % Store the data for this iteration
    timeData(iteration) = simulatedTime;
    explorationRateData(iteration) = explorationRate;


% Create a table with the collected data
data = table(timeData', explorationRateData', 'VariableNames', {'Time', 'ExplorationRate'});

% Specify the Excel file name
excelFileName = 'time_and_exploration_data.xlsx';

% Write the table to an Excel file
writetable(data, excelFileName, 'Sheet', 1);
