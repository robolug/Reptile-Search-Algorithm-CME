...
%     block_width, block_length, block_height; % Vertex 7 (Increased height)
%     0, block_length, block_height; % Vertex 8 (Increased height)
% ];'pentagram'
% 
% faces = [
%     1, 2, 3, 4;  % Bottom face
...
% inflate(map,1)
% show(map)
%%  new code for ambient journal complex case 1
...
%% with 3d effect
% Define different sizes for obstacles
% block_heights = [5, 5, 1]; % Heights for vertical stacks and top block
% block_widths = [1, 1, 1]; % Same width for all
% block_lengths = [1, 1, 7]; % Vertical stacks shorter, top block longer
% 
% % Vertices for a base cuboid (these will be scaled)
% base_vertices = [
%     0, 0, 0;    % Vertex 1
...
% ];
% 
% % Define the positions for the base and top blocks of the bridge
% bridge_base_positions = [
%     10, 10;  % Position for the base block 1
%     10, 20;  % Position for the base block 2
% ];
% 
% bridge_top_position = [10, 15];  % Position for the top block
% 
% % Plot the base blocks
% for i = 1:size(bridge_base_positions, 1)
%     block_height = block_heights(1);
%     block_width = block_widths(1);
...
% 
%     % Plot the block at the specified position
%     patch('Vertices', vertices + repmat([bridge_base_positions(i, :), 0], 8, 1), 'Faces', faces, 'FaceColor', 'k', 'EdgeColor', 'k');
%     hold on;
% 
%     % Set occupancy in the map for the current block
%     setOccupancy(map, bridge_base_positions(i, :), 1);
% end
% 
% % Plot the top block (bridge span)
% block_height = block_heights(3);
% block_width = block_widths(3);
% block_length = block_lengths(3);
% 
% % Scale the base vertices
% vertices = [
%     base_vertices(:, 1) * block_length, ...
...
% ];
% 
% % Plot the block at the specified position with a height offset
% patch('Vertices', vertices + repmat([bridge_top_position, block_heights(1)], 8, 1), 'Faces', faces, 'FaceColor', 'k', 'EdgeColor', 'k');
% 
% % Set occupancy in the map for the current block
% setOccupancy(map, bridge_top_position, 1);
% 
% hold on
% % Set viewpoint
% view(3);
