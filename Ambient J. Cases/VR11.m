...
vertices = [
    0, 0, 0;               % Vertex 1
    block_length, 0, 0;    % Vertex 2 (Increased length)
    block_length, block_width, 0;    % Vertex 3 (Increased length, narrowed width)
    0, block_width, 0;     % Vertex 4 (Narrowed width)
    0, 0, block_height;    % Vertex 5 (Increased height)
    block_length, 0, block_height;   % Vertex 6 (Increased length, increased height)
    block_length, block_width, block_height;   % Vertex 7 (Increased length, narrowed width, increased height)
    0, block_width, block_height;    % Vertex 8 (Narrowed width, increased height)
];
...
block_positions = randi([1, map_size(1)-block_length], num_obstacles, 2);
...
for i=1
    for j=1:nRbt
        if j==1
            robots(j).Itertn(i).Pose = [7,8];
%             robots(j).Itertn(i).Pose = [6,5,0];
        end
        if j==2
            robots(j).Itertn(i).Pose = [15,20];
%             robots(j).Itertn(i).Pose = [7,4,0];
        end
        if j==3
            robots(j).Itertn(i).Pose = [20,8];
