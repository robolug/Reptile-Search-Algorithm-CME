...
%             for z = 0:block_height - 1
%            %     map(y, x, z+1) = 1; % Adjust index to match map(y, x, z) format
%             end
%         end
%     end
% end
...
% 
% % Inflate the map (assuming map is a 3D matrix)
% %map_inflated = 
% inflate(map, 1); % Custom function to inflate map

%%  uniform distribution case
% block_height = 5.0; % Increased height
% block_width = 2.0;   % Increased width
% 
% vertices = [
%     0, 0, 0;            % Vertex 1
%     block_width, 0, 0;  % Vertex 2 (Increased width)
%     block_width, block_width, 0;  % Vertex 3 (Increased width)
%     0, block_width, 0;  % Vertex 4 (Increased width)
%     0, 0, block_height; % Vertex 5 (Increased height)
%     block_width, 0, block_height; % Vertex 6 (Increased height)
%     block_width, block_width, block_height; % Vertex 7 (Increased height)
%     0, block_width, block_height; % Vertex 8 (Increased height)
% ];
% 
% faces = [
%     1, 2, 3, 4;  % Bottom face
...
% ];
% 
% % Define positions for multiple blocks
% block_positions = [
%     11, 5; 12, 5; 13, 5;  % First row
%     10, 22; 10, 21; 10, 20;  % Second row
%     18, 22; 18, 21; 18, 20;  % Third row
%   %  22, 23; 18, 21; 18, 20;
% ];
% 
% % Plot multiple blocks
...
% % Show the map
% show(map)
%%  case 2 figures in ambient journal
% Define block dimensions
block_width = 0.5;   % Narrower width
block_length = 5.0;  % Longer length
block_height = 3.0;  % Increased height

% Vertices for a block
vertices = [
    0, 0, 0;                        % Vertex 1
    block_length, 0, 0;             % Vertex 2
    block_length, block_width, 0;   % Vertex 3
    0, block_width, 0;              % Vertex 4
    0, 0, block_height;             % Vertex 5
    block_length, 0, block_height;  % Vertex 6
    block_length, block_width, block_height; % Vertex 7
    0, block_width, block_height;   % Vertex 8
];
...
% Define specific positions for obstacles
block_positions = [
    5, 5;
    10, 15;
    15, 10;
...
% map_size = [30, 30]; % Assuming a 30x30 grid
% map = robotics.OccupancyGrid(map_size(1), map_size(2), 1);
...
%    10,22;10,21;10,20;10,19;10,18;10,17;10,16;10,16;10,15;10,14;
%    18,22; 18,21; 18,20; 18,19; 18,18 ;18,17; 18,16; 18,15; 18,14;
% ];
% 
% % Plot multiple blocks
% for i = 1:size(block_positions, 1)
...
for i=1
    for j=1:nRbt
        if j==1
           % robots(j).Itertn(i).Pose = [4,27]; % black
             robots(j).Itertn(i).Pose = [4,5];
        end
        if j==2
     %       robots(j).Itertn(i).Pose = [27,27]; % blue
            robots(j).Itertn(i).Pose = [14,20];
        end
        if j==3
       %     robots(j).Itertn(i).Pose = [27,4];  % red
             robots(j).Itertn(i).Pose = [20,4];
        end
...
%          thi = robots(j).Itertn(i).Pose(1,3);        
        ...
        if j==1
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(1),'marker','pentagram');
            hold on
        elseif j==2
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(2),'marker','pentagram');
            hold on
        elseif j==3
            plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(3),'marker','pentagram');
            hold on
...
%             C3 = 2*rand(varSize);
  ...
  t=1;          
    ...
    Beta=2*a*rand(varSize);                  % the best value 0.005
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    reptile(j).Itertn(i).crocodile2.X2 =  getOccupancy(map,reptile(j).Itertn(i).crocodile2.Position)* P*rand;

      %  else 
    reptile(j).Itertn(i).crocodile3.X3 =  getOccupancy(map,reptile(j).Itertn(i).crocodile3.Position) -Eta.*eps-R.*rand;
 
       %    end 
        
          
          
  temp_arr = [reptile(j).Itertn(i).crocodile.X1, reptile(j).Itertn(i).crocodile2.X2...
      reptile(j).Itertn(i).crocodile3.X3];
...
%            ES = 2*randn - (1-t/nIter)))
        ...
        for j=1:nRbt
                ...
                if j==1
                    plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(1),'marker','pentagram');
                    hold on

                elseif j==2
                    plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(2),'marker','pentagram');
                    hold on

                elseif j==3
                    plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(3),'marker','pentagram');
                    hold on
...
for j=1:nRbt
        for it_=1:i
            ...
            if j==1
                    plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),'color',color(1),'marker','pentagram');
                    hold on

                elseif j==2
                    plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),'color',color(2),'marker','pentagram');
                    hold on

                elseif j==3
                    plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),'color',color(3),'marker','pentagram');
                    hold on
