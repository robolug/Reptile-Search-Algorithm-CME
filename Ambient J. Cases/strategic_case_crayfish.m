...
%%
% block_height = 4; % Increase the height 0.35
% vertices = [
%     0, 0, 0;            % Vertex 1
%     1.5, 0, 0;          % Vertex 2
%     1.5, 1.5, 0;        % Vertex 3
%     0, 1.5, 0;          % Vertex 4
%     0, 0, block_height; % Vertex 5 (Increased height)
%     1.5, 0, block_height;% Vertex 6 (Increased height)
%     1.5, 1.5, block_height;% Vertex 7 (Increased height)
%     0, 1.5, block_height;% Vertex 8 (Increased height)
% ];
% 
% faces = [
%     1, 2, 3, 4;  % Bottom face
...
% ];
% 
% % Define fixed positions for the blocks
% %p1 = [11,17]; p2 =[12,16]; p3 =[13,17]; p4=[14,16]; p5=[15,11];
% p1 = [1,17]; p2 = [2,17]; p3 = [3,17]; p4 = [4,17]; p5 = [5,17]; p6 = [6,17];
% 
% p7=[6,16]; p8 =[6,15]; p9=[6,14]; p10=[6,13]; p11=[6,12]; p12=[6,11]; p13=[6,10];
% p14=[6,9]; p15=[6,8]; p16 =[7.5,8]; p17 =[8.5,8];p18=[10,8]; p19=[11,8]; p20=[12,8]; 
% 
% positions = [p1; p2; p3; p4; p5;p6 ;p7;p8; p9;p10;p11;p12;p13;p14;p15;p16;p17;p18;p19;p20];
% %positions_ = [11, 17, 0; 12, 16, 0; 13,17,0 ;14, 16,0; 15,11,0];  % Adjust positions as needed
% 
% positions_ = [1, 17, 0; 2,17, 0; 3,17,0 ;4, 17,0; 5,17,0; 6,17,0 ; 6,16,0; 6,15,0; 6,14,0;...
%     6,13,0; 6,12,0; 6,11,0; 6,10,0; 6,9,0; 6,8,0; 7.5,8,0;8.5,8,0; 10,8,0; 11,8,0; 12,8,0];  % Adjust positions as needed
% 
% num_blocks = size(positions_, 1);
% 
% for i = 1:num_blocks
%     % Plot the block at the specified position
%     patch('Vertices', vertices + repmat(positions_(i, :), 8, 1), 'Faces', faces, 'FaceColor', 'k', 'EdgeColor', 'k');
%     hold on;
% 
%     % Set occupancy in the map for the current block
%     setOccupancy(map, [positions(i, 1), positions(i, 2)], 1);
% end
% 
% hold on
% % Set viewpoint
...
%angles = linspace(3.14, -3.13, 100); % used in raybeam, to draw a full circle around the robot
 angles = linspace(2*pi, 200);
maxrange = 5;
...
Utility = ones(bound);
 ...
 crayfish_empty.Itertn.cray1.Position = [];
 crayfish_empty.Itertn.cray1.Cost = [];
 crayfish_empty.Itertn.cray1.Index = [];
 crayfish_empty.Itertn.cray2.Position = [];
 crayfish_empty.Itertn.cray2.Cost = [];
 crayfish_empty.Itertn.cray2.Index = [];% 
 crayfish_empty.Itertn.cray3.Position = [];
 crayfish_empty.Itertn.cray3.Cost = [];
 crayfish_empty.Itertn.cray3.Index = [];% 

crayfishX_empty.Itertn.Pairs.X = [];
 crayfishX_empty.Itertn.Pairs.Combination = [];
 crayfishX_empty.Itertn.Pairs.Cost = [];
crayfishX = repmat(crayfishX_empty,nRbt,1);
crayfish = repmat(crayfish_empty,nRbt,1);
...
for i=1
    for j=1:nRbt
        if j==1
            robots(j).Itertn(i).Pose = [10,14];   %color = ['k','m','c']; % yellow
  %           robots(j).Itertn(i).Pose = [20,15];
        end
        if j==2
            robots(j).Itertn(i).Pose = [11,13];  % cyan 
            % robots(j).Itertn(i).Pose = [5,20];
        end
        if j==3
            robots(j).Itertn(i).Pose = [9,13];  % blue
             %robots(j).Itertn(i).Pose = [10,5];
        end
...
%          thi = robots(j).Itertn(i).Pose(1,3);        
        ...
        plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(1),'marker','o','LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 1 1 0], 'MarkerSize',7);
   robotOrientation = quiver(robots(j).Itertn(i).Pose(1,1),...
       robots(j).Itertn(i).Pose(1,2), 0.5*cos(pi/4), 0.5*sin(pi/4), 'Color', 'red');
            hold on
        elseif j==2
           plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(1),'marker','o','LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 0 1 1], 'MarkerSize',7);
   robotOrientation = quiver(robots(j).Itertn(i).Pose(1,1),...
       robots(j).Itertn(i).Pose(1,2), 0.5*cos(pi/4), 0.5*sin(pi/4), 'Color', 'red');
                hold on
        elseif j==3
   plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(1),'marker','o','LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 0 0 1], 'MarkerSize',7);
   robotOrientation = quiver(robots(j).Itertn(i).Pose(1,1),...
       robots(j).Itertn(i).Pose(1,2), 0.5*cos(pi/4), 0.5*sin(pi/4), 'Color', 'red');
            hold on
        end
...
% try 
    ...
    %             Utility(V5(1,1),V5(1,2)),Utility(V6(1,1),V6(1,2)),Utility(V7(1,1),V7(1,2)),Utility(V8(1,1),V8(1,2))];
            ...
            crayfish(j).Itertn(i).cray1.Position = eval(['V' num2str(cray1_index)]);
            crayfish(j).Itertn(i).cray1.Cost = cray1;
            crayfish(j).Itertn(i).cray1.Index =cray1_index;
            
            crayfish(j).Itertn(i).cray2.Position = eval(['V' num2str(cray2_index)]);
            crayfish(j).Itertn(i).cray2.Cost = cray2;
            crayfish(j).Itertn(i).cray2.Index = cray2_index;
            
            crayfish(j).Itertn(i).cray3.Position = eval(['V' num2str(cray3_index)]);
            crayfish(j).Itertn(i).cray3.Cost = cray3;
            crayfish(j).Itertn(i).cray3.Index = cray3_index;
...
xshade = (cray1 +cray2+ cray3)/3;
  ...
  crayfish(j).Itertn(i).cray1.X1 = getOccupancy(map,crayfish(j).Itertn(i).cray1.Position)+...
  A1.*rand(1,dim)*(xshade-(getOccupancy(map,robots(j).Itertn(i).Pose(1,:))));
% Xnew(i,:) = X(i,:)+C*rand(1,dim).*(xshade-X(i,:));

       
 crayfish(j).Itertn(i).cray2.X2 = getOccupancy(map,crayfish(j).Itertn(i).cray2.Position)-...
     getOccupancy(map,robots(j).Itertn(i).Pose(1,:))+xshade;
%  Xnew(i,j) = X(i,j)-X(z,j)+xshade(j);
    
        
%  Xnew(i,j) = X(i,j)+cos(2*pi*rand)*Xfood(j)*p_obj(temp)-sin(2*pi*rand)*Xfood(j)*p_obj(temp);
  P = (3*rand*getOccupancy(map,crayfish(j).Itertn(i).cray3.Position))/getOccupancy(map,robots(j).Itertn(i).Pose(1,:));

 % crayfish(j).Itertn(i).cray3.X3 =  getOccupancy(map,crayfish(j).Itertn(i).cray3.Position) + cos(2*pi*rand)*...
  %     getOccupancy(map,robots(j).Itertn(i).Pose(1,:))*p_obj(temp)-sin(2*pi*rand)*...
  %      getOccupancy(map,robots(j).Itertn(i).Pose(1,:))*p_obj(temp);
        
crayfish(j).Itertn(i).cray3.X3 =  getOccupancy(map,crayfish(j).Itertn(i).cray3.Position)-getOccupancy(map,robots(j).Itertn(i).Pose(1,:))*...
    p_obj(temp)+p_obj(temp).*rand(1,dim)*getOccupancy(map,crayfish(j).Itertn(i).cray3.Position).*P;
   
   %    Xnew(i,:) = (X(i,:)-Xfood)*p_obj(temp)+p_obj(temp).*rand(1,dim).*X(i,:);  
          
          
  temp_arr = [crayfish(j).Itertn(i).cray1.X1, crayfish(j).Itertn(i).cray2.X2, crayfish(j).Itertn(i).cray3.X3]
                [value, index] = max(temp_arr);
                crayfish(j).Itertn(i).best.index = index;
                crayfish(j).Itertn(i).best.value = value;

             if index == 1
                x = crayfish(j).Itertn(i).cray1.Position(1,1);
                y = crayfish(j).Itertn(i).cray1.Position(1,2);
                disp("Robot " + j +" selected cray1 position");
 
                   elseif index == 2
                       x = crayfish(j).Itertn(i).cray2.Position(1,1);
                        y = crayfish(j).Itertn(i).cray2.Position(1,2);
                       disp("Robot " + j + " selected cray2 position");
                  elseif index == 3
                       x = crayfish(j).Itertn(i).cray3.Position(1,1);
                       y = crayfish(j).Itertn(i).cray3.Position(1,2);
                       disp("Robot " + j + " selected cray3 position");
...
%            ES = 2*randn - (1-t/nIter)))
        ...
        for j=1:nRbt
                ...
                if j==1
                  %  plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),'color',color(1),'marker','x',...
                   %      'LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 1 1 0], 'MarkerSize',9);
     plot(robots(j).Itertn(i).Pose(1,1),robots(j).Itertn(i).Pose(1,2),...
                'color',color(1),'marker','o','LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 1 1 0], 'MarkerSize',7);
   robotOrientation = quiver(robots(j).Itertn(i).Pose(1,1),...
       robots(j).Itertn(i).Pose(1,2), 0.5*cos(pi/4), 0.5*sin(pi/4), 'Color', 'red'); 
...
for j=1:nRbt
        for it_=1:i
            ...
            if j==1
        plot(robots(j).Itertn(it_).Pose(1,1),robots(j).Itertn(it_).Pose(1,2),...
                'color',color(1),'marker','o','LineWidth',0.001,'MarkerEdgeColor','k', 'MarkerFaceColor',[ 1 1 0], 'MarkerSize',7);
   robotOrientation = quiver(robots(j).Itertn(it_).Pose(1,1),...
       robots(j).Itertn(it_).Pose(1,2), 0.5*cos(pi/4), 0.5*sin(pi/4), 'Color', 'red');
